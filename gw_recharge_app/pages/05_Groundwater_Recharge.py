...
with col1:
    st.image('gw_recharge_app/assets/images/water_balance_crop.png', caption='Fig.1: Water balance of the root zone (modified after FAO Irrigation and Drainage Paper No. 56)')            
with col2:
    st.markdown('''            
...
ax21.bar(data.index, gw_recharge_Haude, color='forestgreen', label='ETP: Haude')
ax21.set_xticks([])
...
ax22.bar(data.index, gw_recharge_Oudin, color='darkorange', label='ETP: Oudin')
ax22.set_xticks([])
ax22.set_ylim(0,max_gwr)
ax22.legend(loc='upper left', frameon=False)
ax22.yaxis.tick_right()

ax23.bar(data.index, gw_recharge_PM, color='mediumpurple', label='ETP: Pennman-Monteith')
ax23.set_xticks([])
...
ax24.plot(data.index, gw_recharge_Haude_cum, color='forestgreen')
ax24.plot(data.index, gw_recharge_Oudin_cum, color='darkorange')
ax24.plot(data.index, gw_recharge_PM_cum, color='mediumpurple')
